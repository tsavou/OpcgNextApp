...
test.describe("Parcours Critique : Gestion de Collection", () => {
  ...
  test("Un utilisateur peut se connecter, ajouter une carte et la retirer", async ({
    page,
  }) => {
    // --- ÉTAPE 1 : CONNEXION ---
    ...
    await test.step("Ouverture de la modale et ajout", async () => {
      ...
      await modal
        .getByRole("button", { name: /ajouter à ma collection/i })
        .click();
    });
    ...
    await test.step("Vérification bouton Possédée", async () => {
      await expect(
        page.getByText("Ajouter à la collection", { exact: true }),
      ).not.toBeVisible();

      await expect(
        page.getByRole("button", { name: /possédée|owned/i }),
