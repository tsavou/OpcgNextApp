...
export function CardItem({
  ...
  return (
    <Link
      ...
      className={`group relative cursor-pointer rounded-lg bg-slate-700/30 transition-all`}
    >
      {collectionDetails && (
        <div
          className="absolute -top-2 left-0 z-20 w-fit"
          onClick={(e) => e.stopPropagation()}
        >
          <CollectionInfoBadge collectionDetails={collectionDetails} />
        </div>
      )}

      <div
        className="absolute -top-2 right-0 z-20 opacity-100 transition-opacity"
        onClick={(e) => e.stopPropagation()}
      >
        <CardCollectionToggle card={card} isOwnedInitial={isOwned} />
      </div>
